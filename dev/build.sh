...
export APP_NAME="THE YVN CODE"
export ASSETS_REPOSITORY="the-yvn/the-yvn-code"
export BINARY_NAME="yvncode"
export CI_BUILD="no"
export GH_REPO_PATH="the-yvn/the-yvn-code"
export ORG_NAME="THE YVN"
...
while getopts ":ilops" opt; do
  case "$opt" in
    i)
      export ASSETS_REPOSITORY="the-yvn/the-yvn-code-insiders"
      export BINARY_NAME="yvncode-insiders"
...
if [[ "${SKIP_BUILD}" == "no" ]]; then
  ...
  if [[ -f "./include_${OS_NAME}.gypi" ]]; then
    ...
    if [[ -f "${HOME}/.gyp/include.gypi" ]]; then
      mv ~/.gyp/include.gypi ~/.gyp/include.gypi.pre-yvncode
    else
      echo "{}" > ~/.gyp/include.gypi.pre-yvncode
    fi
  ...
  if [[ -f "./include_${OS_NAME}.gypi" ]]; then
    mv ~/.gyp/include.gypi.pre-yvncode ~/.gyp/include.gypi
  fi
